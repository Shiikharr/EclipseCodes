...
public class Key_present_in_every_segment {
...
Output : No
	...
	{
		Key_present_in_every_segment ob=new Key_present_in_every_segment();
		int a[]= { 3, 5, 2, 4, 9, 3, 1, 7, 3, 11, 12, 3} ;
		int x=3;
		int k=3;
		ob.check(a,x,k);
		int b[]={ 21, 23, 56, 65, 34, 54, 76, 32, 23, 45, 21, 23, 25};
		ob.check(b, 23, 5);
		int c[]= { 5, 8, 7, 12, 14, 3, 9} ;
		ob.check(c, 8, 2);
	}

	private void check(int[] a, int x, int k) {
		int n=a.length;int j=0;
